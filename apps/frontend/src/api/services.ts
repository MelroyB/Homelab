...
export function validateConfig(
  ...
  return apiRequest<ConfigValidationResponse>(
    `/services/${slug}/configs/validate`,
    {
      method: "POST",
...
export function rollbackConfig(slug: string, versionId: string) {
  return apiRequest<ConfigVersion>(
    `/services/${slug}/configs/${versionId}/rollback`,
    {
      method: "POST"
    }
  );
...
export function getServiceLogs(
  slug: string,
  tail = 200
): Promise<{ service: string; logs: string }> {
  return apiRequest<{ service: string; logs: string }>(
    `/services/${slug}/logs?tail=${tail}`
  );
}
